...
def find_best_match(top_x, transplants_file_path, year, loci_list, donor_type):
    ...
    while don_order:
        ...
        max_hla_score, pat_idx, don_idx, old_order = 1000, 0 , 0, 0
        ...
        if order - top_10[0][1]   > top_x -1 and pat_param["blood"] in blood_2_blood[don_param["blood"]] and \
            ...
            if max_hla_score == 1000: # if no one match by hla, take the first that match by blood and age
        ...
        if max_hla_score != 1000:
            ...
            total_hla_score+= 2*(len(loci_list))
    ...
    std = list_val.std()
    return round(total_hla_score/size, 2), size, std, unmatched_donors_count
    ...
    for donor_type in [ "dead"]:
        ...
        for k in range(0,20):
            ...
            for i, loci_list in enumerate([["A", "B", "DRB1"]]):
                ...
                m = find_best_match(k+1, transplants_file_path,  year, loci_list, donor_type)
                list_res.append(str(m[0]))
                list_res.append(str(m[1]))
                list_res.append(str(m[2]))
